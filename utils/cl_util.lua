...
function tnexa.getStreetNameAtCoord(v, w, H)
...
function tnexa.randomNum(d, e)
    math.randomseed(GetGameTimer() * math.random() * 2)
    return math.random(d, e)
end
soundEventCode = 0
TriggerServerEvent("nexa:soundCodeServer")
RegisterNetEvent("nexa:soundCode",function(aH)
    soundEventCode = aH
end)
RegisterNetEvent("nexa:playClientNuiSound",function(aI, aJ, ap) -- use for cuffing and other sounds in areas
    local ax = tnexa.getPlayerCoords()
    if #(ax - aI) <= ap then
        SendNUIMessage({transactionType = aJ})
    end
end)
function tnexa.getNetId(aW, aX)
    if aX == nil then
        aX = ""
    end
    local aY = 0
    ...
    if not aZ then
        tnexa.debugLog(string.format("no such entity %s", aX))
    else
        aY = NetworkGetNetworkIdFromEntity(aW)
        if aY == aW then
            tnexa.debugLog(string.format("no such networked entity %s", aX))
        end
    end
    return aY
end
function tnexa.getObjectId(a_, aX)
    if aX == nil then
        aX = ""
    end
    local aL = 0
    ...
    if not b0 then
        tnexa.debugLog(string.format("no object by ID %s\n%s", a_, aX))
    else
  ...
  local aU = {}
  Citizen.CreateThread(function()
    local a = module("nexa-vehicles", "garages")
...
function tnexa.getVehicleNameFromId(aZ)
    if aT[string.lower(aZ)] then
        return aT[string.lower(aZ)].name
    end
    return ""
end
function tnexa.getGarageNameFromId(aZ)
    return aT[string.lower(aZ)].garageType
end
function tnexa.getVehicleIdFromModel(s)
    return aU[s]
end
local a_ = math.rad
local b0 = math.cos
local b1 = math.sin
local b2 = math.abs
function tnexa.rotationToDirection(b3)
    local B = a_(b3.x)
    local D = a_(b3.z)
