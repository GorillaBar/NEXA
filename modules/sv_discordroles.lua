...
local function Get_Client_Discord_ID(source)
	return exports['ghmattimysql']:executeSync("SELECT discord_id FROM `nexa_verification` WHERE user_id = @user_id", {user_id = nexa.getUserId(source)})[1].discord_id
end
...
local function Modify_Client_Roles(guild_name, discord_id, user_id)
	...
	if discord_roles then
		...
		for _, role_id in pairs(does_not_have_roles) do
			for k,v in pairs(cfg.Guild_Roles[guild_name]) do
                if v == role_id and nexa.hasGroup(user_id, k) then
					print(k)
                    nexa.removeUserGroup(user_id, k)
					if nexa.hasGroup(user_id, k..' Clocked') then
						nexa.removeUserGroup(user_id, k..' Clocked')
					end
                end
			end
		end
		...
		for _, role_id in pairs(has_roles) do
            for k,v in pairs(cfg.Guild_Roles[guild_name]) do
                if v == role_id and not nexa.hasGroup(user_id, k) then
                    nexa.addUserGroup(user_id, k)
                end
            end
		end
	else
	...
	nexa.getJobSelectors(nexa.getUserSource(user_id))
end

local tracked = {}
RegisterNetEvent('nexa:getFactionWhitelistedGroups')
AddEventHandler('nexa:getFactionWhitelistedGroups', function()
	local source = source
	tnexa.getFactionGroups(source)
end)

function tnexa.getFactionGroups(source)
    local source = source
...
local function Get_Guild_Username(guild_id, discord_id)
	local endpoint = ("guilds/%s/members/%s"):format(guild_id, discord_id)
	local member = DiscordRequest("GET", endpoint, {})
	if member.code == 200 then
		local data = json.decode(member.data)
...
exports('Get_Guild_Username', function(guild_id, discord_id)
	return Get_Guild_Username(guild_id, discord_id)
end)
...
local function Get_Guild_Status(guild)
	if guild.code == 200 then
		...
		print(("^2Successful connection to Guild: ^4%s ^5(%s)^0"):format(data.name, data.id))
  	else
...
function tnexa.checkForRole(user_id, role_id)
...
function nexa.getDiscordIdFromSource(source)
	return Get_Client_Discord_ID(source)
end
