...
RegisterCommand('carryaccept', function(source)
    if requests[source] then
        ...
        nexaclient.notify(requests[source], {"~g~Request Accepted"})
...
RegisterCommand('carryrefuse', function(source)
    if requests[source] then
        nexaclient.notify(requests[source], {"~r~Request Refused"})
        requests[source] = nil
    end
end)
...
AddEventHandler("CarryPeople:requestCarry", function(targetSrc)
    ...
    if targetSrc ~= nil then
        ...
        nexaclient.notify(source, {"~g~Request sent to temp id "..targetSrc})
    end
end)
