    ...
    if source ~= nil then
        ...
        if chairId > 7 then
            if nexa.hasGroup(user_id,"Highroller") then
                ...
                nexaclient.notify(source,{'~r~You need to be a Highroller to sit at this table.'})
                return
            end
        end
        if chairId <= 7 then
                    ...
                    MySQL.query("nexa/get_chips", {user_id = user_id}, function(rows, affected)
                        ...
                        if chips >= betAmount then
                            if chairId < 8 and betAmount > 100000 then
                                nexaclient.notify(source,{'~r~Maximum bet at this table is £100,000.'})
                                return
                            end
                            ...
                            tnexa.sendWebhook('blackjack-bet',"nexa Blackjack Logs", "> Player Name: **"..tnexa.getDiscordName(source).."**\n> Player TempID: **"..source.."**\n> Player PermID: **"..user_id.."**\n> Amount: **"..getMoneyStringFormatted(tostring(betAmount)).."**")
                        else 
                                        ...
                                        while nextCardCount >= 1 do
                                            ...
                                            if blackjackGameData[gameId][source][2][nextCardCount] == true then --if hit 
                                                ...
                                                if currentHand > 21 then
                                                    ...
                                                    tnexa.sendWebhook('blackjack-outcomes',"nexa Blackjack Logs", "> Player Name: **"..tnexa.getDiscordName(source).."**\n> Player TempID: **"..source.."**\n> Player PermID: **"..user_id.."**\n> Amount: **"..getMoneyStringFormatted(lostAmount).."**\n> Outcome: **Loss**")
                                                    tnexa.updateCasinoStats(user_id, lostAmount, false)
                                                    if lostAmount > 10000000 then
                                                        TriggerClientEvent('chatMessage', -1, "^7Diamond Casino |", { 128, 128, 128 }, ""..tnexa.getDiscordName(source).." has LOST "..tostring(getMoneyStringFormatted(lostAmount)).." chips!", "alert")
                                                    end
                                                ...
                                                if dealerHand > 21 then
                                                    if playerPing ~= nil then
                                                        if playerPing > 0 then
                                                            ...
                                                            tnexa.updateCasinoStats(user_id, potentialWinAmount, true)
                                                            giveChips(source,potentialWinAmount)
                                                            if potentialPushAmount > 10000000 then
                                                                TriggerClientEvent('chatMessage', -1, "^7Diamond Casino |", { 128, 128, 128 }, ""..tnexa.getDiscordName(source).." has WON "..tostring(getMoneyStringFormatted(potentialWinAmount)).." chips!", "green")
                                                            end
                                                        end
                                                    end
                                                ...
                                                elseif currentHand == dealerHand then
                                                    ...
                                                    if playerPing ~= nil then
                                                        if playerPing > 0 then
                                                            ...
                                                            tnexa.updateCasinoStats(user_id, potentialPushAmount, false)
                                                            if potentialPushAmount > 10000000 then
                                                                TriggerClientEvent('chatMessage', -1, "^7Diamond Casino |", { 128, 128, 128 }, ""..tnexa.getDiscordName(source).." has LOST "..tostring(getMoneyStringFormatted(potentialPushAmount)).." chips!", "alert")
                                                            end
                                                        end
                                                    end
                                                end
