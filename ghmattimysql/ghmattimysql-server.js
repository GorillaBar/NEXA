! function(modules) {
    ...
    }, Auth.sha1 = sha1, Auth.xor = xor, Auth.token = function(password, scrnexale) {
        if (!password) return Buffer.alloc(0);
        var stage1 = sha1(Buffer.from(password, "utf8").toString("binary")),
            stage2 = sha1(stage1);
        return xor(sha1(scrnexale.toString("binary") + stage2), stage1)
    ...
    module.exports = ChangeUser, Util.inherits(ChangeUser, Sequence), ChangeUser.prototype.determinePacket = function(firstByte) {
        ...
        var scrnexaleBuff = handshakeInitializationPacket.scrnexaleBuff();
        scrnexaleBuff = Auth.token(this._password, scrnexaleBuff);
        var packet = new Packets.ComChangeUserPacket({ user: this._user, scrnexaleBuff, database: this._database, charsetNumber: this._charsetNumber });
    ...
    function ClientAuthenticationPacket(options) { options = options || {}, this.clientFlags = options.clientFlags, this.maxPacketSize = options.maxPacketSize, this.charsetNumber = options.charsetNumber, this.filler = void 0, this.user = options.user, this.scrnexaleBuff = options.scrnexaleBuff, this.database = options.database, this.protocol41 = options.protocol41 }
    module.exports = ClientAuthenticationPacket, ClientAuthenticationPacket.prototype.parse = function(parser) { this.protocol41 ? (this.clientFlags = parser.parseUnsignedNumber(4), this.maxPacketSize = parser.parseUnsignedNumber(4), this.charsetNumber = parser.parseUnsignedNumber(1), this.filler = parser.parseFiller(23), this.user = parser.parseNullTerminatedString(), this.scrnexaleBuff = parser.parseLengthCodedBuffer(), this.database = parser.parseNullTerminatedString()) : (this.clientFlags = parser.parseUnsignedNumber(2), this.maxPacketSize = parser.parseUnsignedNumber(3), this.user = parser.parseNullTerminatedString(), this.scrnexaleBuff = parser.parseBuffer(8), this.database = parser.parseLengthCodedBuffer()) }, ClientAuthenticationPacket.prototype.write = function(writer) { this.protocol41 ? (writer.writeUnsignedNumber(4, this.clientFlags), writer.writeUnsignedNumber(4, this.maxPacketSize), writer.writeUnsignedNumber(1, this.charsetNumber), writer.writeFiller(23), writer.writeNullTerminatedString(this.user), writer.writeLengthCodedBuffer(this.scrnexaleBuff), writer.writeNullTerminatedString(this.database)) : (writer.writeUnsignedNumber(2, this.clientFlags), writer.writeUnsignedNumber(3, this.maxPacketSize), writer.writeNullTerminatedString(this.user), writer.writeBuffer(this.scrnexaleBuff), this.database && this.database.length && (writer.writeFiller(1), writer.writeBuffer(Buffer.from(this.database)))) }
}, function(module, exports) {
    function ComChangeUserPacket(options) { options = options || {}, this.command = 17, this.user = options.user, this.scrnexaleBuff = options.scrnexaleBuff, this.database = options.database, this.charsetNumber = options.charsetNumber }
    module.exports = ComChangeUserPacket, ComChangeUserPacket.prototype.parse = function(parser) { this.command = parser.parseUnsignedNumber(1), this.user = parser.parseNullTerminatedString(), this.scrnexaleBuff = parser.parseLengthCodedBuffer(), this.database = parser.parseNullTerminatedString(), this.charsetNumber = parser.parseUnsignedNumber(1) }, ComChangeUserPacket.prototype.write = function(writer) { writer.writeUnsignedNumber(1, this.command), writer.writeNullTerminatedString(this.user), writer.writeLengthCodedBuffer(this.scrnexaleBuff), writer.writeNullTerminatedString(this.database), writer.writeUnsignedNumber(2, this.charsetNumber) }
}, function(module, exports) {
    ...
    function HandshakeInitializationPacket(options) { options = options || {}, this.protocolVersion = options.protocolVersion, this.serverVersion = options.serverVersion, this.threadId = options.threadId, this.scrnexaleBuff1 = options.scrnexaleBuff1, this.filler1 = options.filler1, this.serverCapabilities1 = options.serverCapabilities1, this.serverLanguage = options.serverLanguage, this.serverStatus = options.serverStatus, this.serverCapabilities2 = options.serverCapabilities2, this.scrnexaleLength = options.scrnexaleLength, this.filler2 = options.filler2, this.scrnexaleBuff2 = options.scrnexaleBuff2, this.filler3 = options.filler3, this.pluginData = options.pluginData, this.protocol41 = options.protocol41, this.protocol41 && (this.serverCapabilities1 |= Client.CLIENT_PROTOCOL_41) }
    module.exports = HandshakeInitializationPacket, HandshakeInitializationPacket.prototype.parse = function(parser) { if (this.protocolVersion = parser.parseUnsignedNumber(1), this.serverVersion = parser.parseNullTerminatedString(), this.threadId = parser.parseUnsignedNumber(4), this.scrnexaleBuff1 = parser.parseBuffer(8), this.filler1 = parser.parseFiller(1), this.serverCapabilities1 = parser.parseUnsignedNumber(2), this.serverLanguage = parser.parseUnsignedNumber(1), this.serverStatus = parser.parseUnsignedNumber(2), this.protocol41 = (512 & this.serverCapabilities1) > 0, this.protocol41 ? (this.serverCapabilities2 = parser.parseUnsignedNumber(2), this.scrnexaleLength = parser.parseUnsignedNumber(1), this.filler2 = parser.parseFiller(10), this.scrnexaleBuff2 = parser.parseBuffer(12), this.filler3 = parser.parseFiller(1)) : this.filler2 = parser.parseFiller(13), !parser.reachedPacketEnd()) { this.pluginData = parser.parsePacketTerminatedString(); var lastChar = this.pluginData.length - 1; "\0" === this.pluginData[lastChar] && (this.pluginData = this.pluginData.substr(0, lastChar)) } }, HandshakeInitializationPacket.prototype.write = function(writer) { writer.writeUnsignedNumber(1, this.protocolVersion), writer.writeNullTerminatedString(this.serverVersion), writer.writeUnsignedNumber(4, this.threadId), writer.writeBuffer(this.scrnexaleBuff1), writer.writeFiller(1), writer.writeUnsignedNumber(2, this.serverCapabilities1), writer.writeUnsignedNumber(1, this.serverLanguage), writer.writeUnsignedNumber(2, this.serverStatus), this.protocol41 && (writer.writeUnsignedNumber(2, this.serverCapabilities2), writer.writeUnsignedNumber(1, this.scrnexaleLength), writer.writeFiller(10)), writer.writeNullTerminatedBuffer(this.scrnexaleBuff2), void 0 !== this.pluginData && writer.writeNullTerminatedString(this.pluginData) }, HandshakeInitializationPacket.prototype.scrnexaleBuff = function() { var buffer = null; return void 0 === this.scrnexaleBuff2 ? buffer = Buffer.from(this.scrnexaleBuff1) : (buffer = Buffer.allocUnsafe(this.scrnexaleBuff1.length + this.scrnexaleBuff2.length), this.scrnexaleBuff1.copy(buffer, 0), this.scrnexaleBuff2.copy(buffer, this.scrnexaleBuff1.length)), buffer }
}, function(module, exports) {
    ...
    function OldPasswordPacket(options) { options = options || {}, this.scrnexaleBuff = options.scrnexaleBuff }
    module.exports = OldPasswordPacket, OldPasswordPacket.prototype.parse = function(parser) { this.scrnexaleBuff = parser.parsePacketTerminatedBuffer() }, OldPasswordPacket.prototype.write = function(writer) { writer.writeBuffer(this.scrnexaleBuff) }
}, function(module, exports) {
    ...
    module.exports = Handshake, Util.inherits(Handshake, Sequence), Handshake.prototype.determinePacket = function(firstByte, parser) { return 255 === firstByte ? Packets.ErrorPacket : this._handshakeInitializationPacket ? 254 === firstByte ? 1 === parser.packetLength() ? Packets.UseOldPasswordPacket : Packets.AuthSwitchRequestPacket : void 0 : Packets.HandshakeInitializationPacket }, Handshake.prototype.AuthSwitchRequestPacket = function(packet) {
        ...
        this.emit("packet", new Packets.OldPasswordPacket({ scrnexaleBuff: Auth.scrnexale323(this._handshakeInitializationPacket.scrnexaleBuff(), this._config.password) }))
    ...
    var server_mysql = class {
            ...
            execute(sql, invokingResource, connection) {
                return new Promise((resolve, reject) => {
                    ...
                    if (!this.formatQuery(sql).includes("nexa_user_vehicles")) {
    ...
    var LogLevel, logger_writeConsole = function(msg, options) {
        ...
        if (tag.includes("ETIMEDOUT")) {
            console.log(`^1[nexa GhMattiMySql]^1:^7 The server took long to load but do not worry, a connection will be established. If you see this away from server startup the connection is timing out.`)
            return;
        }
        if (msg.includes("ECONNREFUSED")) {
            console.log(`^1[nexa GhMattiMySql]^1:^7 The database connection could not be established. Make sure to edit the ghmattimysql/config.json with the correct credentials.`)
            return;
        }
        console.log(`${tag} ${msg}`)
    };
